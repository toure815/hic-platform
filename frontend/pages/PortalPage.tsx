...
export function PortalPage() {
  ...
  // TEMP DEBUG: test Encore backend health endpoint
  async function testEncoreConnection() {
    try {
      // @ts-ignore: import.meta.env typing may not be declared in this project
      const VITE_API_BASE_URL = import.meta.env.VITE_API_BASE_URL;

      const res = await fetch(`${VITE_API_BASE_URL}/health`, {
        method: "GET",
        credentials: "include",
      });

      const text = await res.text();
      console.log("[TEMP DEBUG] Encore /health status=", res.status);
      console.log("[TEMP DEBUG] Encore /health body=", text);
    } catch (err) {
      ...
      <Card className="border-blue-200 bg-blue-50">
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {/* TEMP DEBUG: visible button to test Encore backend connectivity */}
            <div className="mt-3">
              <Button
                onClick={() => testEncoreConnection()}
                size="sm"
